...
def plot_soil_depletion(days, depletion, raw_mm, taw_mm, irrigation_days, fname):
    """Enlarged, high-contrast version -- bigger figure, thicker lines, bigger fonts,
    filled depletion area, and a shaded 'stress zone' band above RAW so the
    depletion-vs-threshold relationship is immediately visible at a glance."""
    fig, ax = plt.subplots(figsize=(11, 6))
    ax.fill_between(days, raw_mm, taw_mm, color="#f5b7b1", alpha=0.4, zorder=0,
                     label="Stress zone (depletion > RAW)")
    ax.fill_between(days, 0, depletion, color="#8a5a34", alpha=0.18, zorder=1)
    ax.plot(days, depletion, color="#5a3a18", lw=2.6, label="Root-zone depletion (mm)", zorder=3)
    ax.axhline(raw_mm, color="#e67e22", ls="--", lw=2.4, label=f"RAW threshold ({raw_mm:.0f} mm)", zorder=2)
    ax.axhline(taw_mm, color="#c0392b", ls=":", lw=2.4, label=f"TAW ({taw_mm:.0f} mm)", zorder=2)
    for d in irrigation_days:
        ax.axvline(d, color="#1565c0", alpha=0.55, lw=1.8, zorder=2)
    if irrigation_days:
        ax.plot([], [], color="#1565c0", lw=1.8, label="Irrigation event")
    ax.set_xlabel("Day of season", fontsize=12)
    ax.set_ylabel("Depletion (mm)", fontsize=12)
    ax.set_title("Soil-Water Depletion vs RAW Threshold", fontsize=15, fontweight="bold", color="#123524")
    ax.tick_params(labelsize=11)
    ax.legend(fontsize=10, loc="upper left", framealpha=0.9)
    ax.grid(alpha=.35)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    return _save(fig, fname, dpi=200)
