...
def _no_row_split(table):
    """Prevents any single table row from being sliced across a page boundary --
    so if a table doesn't fully fit on the current page, Word moves the whole
    remaining block of unsplit rows down to the next page (rather than awkwardly
    starting a table with only 1-2 rows visible at the very bottom of a page and
    continuing it on the next). Applied to every table in this report."""
...
def _build_header_footer(doc, farm_owner_name: str):
    ...
    try:
        ...
        text_run = hp.add_run("  NaCROP — Nigeria Irrigation DSS")
        text_run.bold = True
...
def generate_full_report(results: dict, downstream: dict, today_multi: dict, season: dict,
    ...
    # ---------------- Global formatting: Times New Roman, justified, 1.0 spacing ----------------
    # Applied once via the Normal/Heading styles so every paragraph added below
    # (via doc.add_paragraph()/add_heading(), which inherit from these styles
    # unless a paragraph explicitly overrides its own alignment, e.g. the
    # centered title page) picks it up automatically -- evenly distributed
    # text between the margins, single (1.0) line spacing, polished serif font.
    ...
    for heading_style_name in ("Heading 1", "Heading 2", "Heading 3", "Title"):
        try:
            hstyle = doc.styles[heading_style_name]
    ...
    # ---------------- Title page ----------------
    title = doc.add_heading("NaCROP — Nigeria Irrigation DSS: Farm Report", level=0)
    ...
    # ---------------- Abstract (condensed, scientific-abstract style; first) ----------------
    ...
    doc.add_paragraph(
        f"This report quantifies crop water requirement, irrigation scheduling, and expected productivity "
        f"for {area_ha} ha of {crop['display_name']} at {farm_owner_name or 'this farm'} near Zaria, "
        f"Kaduna State (Lat {lat:.4f}, Lon {lon:.4f}), generated {datetime.now().strftime('%Y-%m-%d %H:%M')} "
        f"from an entered temperature of {ti['temperature_c']}\u00b0C and humidity of {ti['humidity_pct']}%. "
        f"Reference and crop evapotranspiration were estimated from a temperature/humidity-trained climate "
        f"model, propagated through a daily soil-water balance to size a stage-specific irrigation schedule "
        f"and a system-efficiency-adjusted gross water budget. Predicted crop water use today is "
        f"{results['et']['today_predicted_etc']} mm/day ({results['et'].get('growing_season_etc_mm')} mm "
        f"over the {results['et'].get('growing_season_days')}-day season); net seasonal irrigation "
        f"requirement is {wb_summary.get('net_irrigation_mm')} mm, delivered as "
        f"{rec_summary.get('n_events')} recommended {recommendation['recommended_method'].lower()} events "
        f"at {results['efficiency']['Ep']}% overall system efficiency. The water balance closes to "
        f"{wb_summary.get('balance_residual_mm')} mm residual. Estimated yield is "
        f"{prod_est['estimated_yield_t_ha']} t/ha ({prod_est['estimated_total_production_t']} t total). "
        f"Full methodology, data sources, and every supporting figure and table are given in Parts I-IV below."
    ...
    _heading(doc, "How to Use This Report", level=2)
    guide_items = [
        ("Abstract (above)", "The numbers that matter for a same-day irrigation decision, condensed."),
        ("Part I \u2014 Climate & ET", "Live inputs and how they translate into today's and the season's "
         "crop water use, with a full plain-text data dump for every intermediate number."),
        ("Part II \u2014 Soil Water & Irrigation Management", "Root-zone water-holding capacity, the "
         "practical stage-specific irrigation schedule, where water is lost between source and crop, and "
         "which delivery method fits this farm."),
        ("Part III \u2014 Crop Growth, Simulation & Productivity", "Germination-to-harvest growth "
         "simulation, stage-by-stage field water accounting, estimated yield, what to monitor, the "
         "week-by-week water ledger, and NaCROP's own canopy\u2192transpiration\u2192biomass\u2192yield model."),
        ("Part IV \u2014 Supporting Material", "Extra figures, this farm's own terrain, full methodology "
         "and data sources, glossary, and assumptions/limitations, for a technical reviewer."),
    ]
    table_guide = doc.add_table(rows=1, cols=2)
    table_guide.style = "Light Grid Accent 1"
    hdr = table_guide.rows[0].cells
    hdr[0].text = "Part"; hdr[1].text = "What it's for"
    ...
    for name, desc in guide_items:
        ...
        row[0].text = name
        row[1].text = desc
    _no_row_split(table_guide)

    # ---------------- Site and Climate Normals ----------------
    _heading(doc, "Site and Climate Normals", level=2)
    doc.add_paragraph(
    ...
    _no_row_split(table_clim)

    # ---------------- 1. Overview ----------------
    _heading(doc, "PART I \u2014 CLIMATE & EVAPOTRANSPIRATION", level=1)
    _heading(doc, "1.1 Overview", level=2)
    ...
    # ---------------- 2. Dashboard ----------------
    _heading(doc, "1.2 Dashboard Summary", level=2)
    ...
    _heading(doc, "1.2.1 ET Methods Used", level=3)
    doc.add_paragraph(
    ...
    # ---------------- 3. Soil Water ----------------
    _heading(doc, "PART II \u2014 SOIL WATER & IRRIGATION MANAGEMENT", level=1)
    _heading(doc, "2.1 Soil Water", level=2)
    ...
    _heading(doc, "2.2 Irrigation Schedule", level=2)
    ...
    _heading(doc, "2.3 Efficiency & Water Budget", level=2)
    ...
    _heading(doc, "2.4 Irrigation Method Recommendation", level=2)
    ...
    _heading(doc, "2.5 Water Required by Irrigation Method: This Farm's Area", level=2)
    ...
    _heading(doc, "PART III \u2014 CROP GROWTH, SIMULATION & PRODUCTIVITY", level=1)
    _heading(doc, "3.1 Crop Growth Simulation: Germination to Harvest", level=2)
    ...
    _heading(doc, "3.2 Stage-by-Stage Field Simulation and Water Accounting", level=2)
    ...
    _heading(doc, "3.3 Estimated Productivity: Water-Yield Response Model", level=2)
    doc.add_paragraph(
    ...
    _heading(doc, "3.4 What to Monitor for Maximum Productivity", level=2)
    ...
    _heading(doc, "3.5 Weekly Water Balance Summary", level=2)
    doc.add_paragraph(
    ...
    while i < min(season_days, len(dwb)):
        ...
        week_no += 1
        i += 7
    _no_row_split(table_wk)

    # ---------------- 17. NaCROP Simulation: Canopy -> Transpiration -> Biomass -> Yield ----------------
    _heading(doc, "3.6 NaCROP Simulation: Canopy \u2192 Transpiration \u2192 Biomass \u2192 Yield", level=2)
    doc.add_paragraph(
        "NaCROP's own 4-step crop-growth model: Canopy Cover \u2192 Transpiration \u2192 Biomass \u2192 Yield, "
        "run specifically for this farm using its own crop, climate, and soil-water-balance results "
        "above, rather than generic textbook values."
    )
    sim_context = {}
    sim_result = None
    try:
        from . import simulation as _sim
        from . import simulation_figure as _simfig
        crop_key = crop["key"]
        sim_result = _sim.simulate_nacrop_season(
            crop_key, model.doy_climatology, cfg.DEFAULT_SITE.latitude_deg, farm_area_ha=area_ha,
            dwb_depletion_series=[d.depletion_mm for d in downstream["dwb"]],
            dwb_taw_series=downstream["taw_raw"]["TAW_mm"], ask_questions=False,
        )
        sim_fig_path = _simfig.plot_nacrop_simulation(
            sim_result, fname=f"nacrop_simulation_{(farm_owner_name or 'farm').replace(' ', '_')}.png")
        p = sim_result.params
        _kv_table(doc, [
            ("Crop", sim_result.crop_name), ("Season length", f"{sim_result.season_length_days} days"),
            ("CC0 / CCx", f"{p['CC0_pct']}% / {p['CCx_pct']}%"),
            ("CGC / CDC", f"{p['CGC_per_day']}/day / {p['CDC_per_day']}/day"),
            ("KcTr,x", p['KcTr_x']), ("WP*", f"{p['WP_star_g_m2']} g/m\u00b2"),
            ("Base temperature", f"{p['base_temperature_c']}\u00b0C"),
            ("Final biomass", f"{sim_result.final_biomass_kg_ha:,.0f} kg/ha"),
            ("Reference HIo / Actual HI", f"{sim_result.reference_hi_pct}% / {sim_result.actual_hi_pct}%"),
            ("Simulated yield", f"{sim_result.yield_kg_ha:,.0f} kg/ha ({sim_result.yield_t_ha:.2f} t/ha)"),
            ("Farm total yield", f"\u2248{sim_result.farm_total_yield_kg:,.0f} kg over {area_ha:g} ha"),
        ])
        _add_figure(doc, sim_fig_path,
                    f"NaCROP Simulation \u2014 {sim_result.crop_name}, this farm's own 4-step growth model "
                    f"(default crop-science parameters; the GUI's NaCROP Simulation tab lets you adjust "
                    f"these interactively).", fig_counter)
        water_stress_occurred = sim_result.actual_hi_pct < sim_result.reference_hi_pct - 0.5
        sim_context = {
            "crop_key": crop_key, "crop_display_name": sim_result.crop_name,
            "water_stress_occurred": water_stress_occurred,
            "irrigation_triggered": bool(downstream["schedule"].get("events")),
            "et_method": results.get("et", {}).get("method_used"),
            "base_temperature_c": p['base_temperature_c'], "cc0_pct": p['CC0_pct'], "ccx_pct": p['CCx_pct'],
            "kctr_x": p['KcTr_x'], "wp_star": p['WP_star_g_m2'], "hio_pct": sim_result.reference_hi_pct,
            "hi_pct": sim_result.actual_hi_pct, "actual_hi_pct": sim_result.actual_hi_pct,
            "yield_kg_ha": sim_result.yield_kg_ha, "final_biomass_kg_ha": f"{sim_result.final_biomass_kg_ha:,.0f}",
            "farm_owner": farm_owner_name, "farm_area_ha": area_ha,
            "farm_total_yield_kg": f"{sim_result.farm_total_yield_kg:,.0f}",
        }
    except Exception as e:
        doc.add_paragraph(f"[NaCROP Simulation could not be computed for this run: {e}]")

    # ---------------- 18. How NaCROP Computed This Season ----------------
    _heading(doc, "3.7 How NaCROP Computed This Season", level=2)
    doc.add_paragraph(
        "A single-page summary of how this farm's own climate, soil-water balance, canopy growth, "
        "transpiration, biomass, and final yield all connect \u2014 drawn specifically for this run, with "
        "every number on it taken directly from the sections above rather than a generic illustration."
    )
    try:
        from . import system_diagram as _sysdiag
        soil_ctx = downstream.get("taw_raw", {})
        today_dwb = downstream["dwb"][season.get("dap_today", -1)] if downstream.get("dwb") else None
        zr_series = downstream.get("zr_series")
        today_idx = season.get("dap_today", -1) if season else -1
        zr_today = (zr_series[today_idx] if zr_series and -len(zr_series) <= today_idx < len(zr_series)
                    else None)
        kc_today = ti.get("kc_today")
        maturity_today = max(0.05, min(1.0, kc_today / 1.2)) if kc_today else 0.5

        diagram_context = dict(sim_context)
        diagram_context.setdefault("crop_key", crop["key"])
        diagram_context.setdefault("crop_name", crop["display_name"])
        diagram_context.setdefault("et_method", results.get("et", {}).get("method_used"))
        diagram_context["farm_owner"] = farm_owner_name
        diagram_context["maturity"] = maturity_today
        diagram_context["taw_mm"] = round(soil_ctx.get("TAW_mm", 0), 1) if soil_ctx.get("TAW_mm") else "?"
        diagram_context["raw_mm"] = round(soil_ctx.get("RAW_mm", 0), 1) if soil_ctx.get("RAW_mm") else "?"
        diagram_context["depletion_mm"] = round(today_dwb.depletion_mm, 1) if today_dwb else "?"
        diagram_context["storage_mm"] = round(today_dwb.storage_mm, 1) if today_dwb else "?"
        diagram_context["rainfall_mm"] = round(today_dwb.rainfall_mm, 1) if today_dwb else "?"
        diagram_context["irrigation_mm"] = round(today_dwb.irrigation_mm, 1) if today_dwb else "?"
        diagram_context["deep_percolation_mm"] = (round(today_dwb.deep_percolation_mm, 1)
                                                    if today_dwb else "?")
        diagram_context["eto_mm_day"] = round(results.get("et", {}).get("mean_et0", 0), 2) \
            if results.get("et", {}).get("mean_et0") is not None else "?"
        diagram_context["etc_mm_day"] = round(results.get("et", {}).get("mean_etc", 0), 2) \
            if results.get("et", {}).get("mean_etc") is not None else "?"
        base_t = diagram_context.get("base_temperature_c")
        tmax = ti.get("tmax_reconstructed_c")
        tmin = ti.get("tmin_reconstructed_c")
        if isinstance(base_t, (int, float)) and tmax is not None and tmin is not None:
            diagram_context["gdd_today"] = round(max(((tmax + tmin) / 2) - base_t, 0.0), 1)
        else:
            diagram_context["gdd_today"] = "?"

        diagram_path = _sysdiag.plot_system_diagram(
            diagram_context, fname=f"nacrop_system_diagram_{(farm_owner_name or 'farm').replace(' ', '_')}.png")
        _add_figure(doc, diagram_path,
                    f"How NaCROP computed this season for {crop['display_name']} on {farm_owner_name or 'this farm'} "
                    f"\u2014 climate and soil water balance driving canopy growth, transpiration, biomass, and "
                    f"final yield.", fig_counter)
    except Exception as e:
        doc.add_paragraph(f"[Summary diagram could not be generated for this run: {e}]")

    # ---------------- 8. Additional Figures ----------------
    _heading(doc, "PART IV \u2014 SUPPORTING MATERIAL", level=1)
    _heading(doc, "4.1 Additional Figures", level=2)
    ...
    _heading(doc, "4.2 Study Area: Terrain Characterisation", level=2)
    ...
    _heading(doc, "4.3 Methodology and Data Sources", level=2)
    doc.add_paragraph(
    ...
    # ---------------- 16. Glossary ----------------
    _heading(doc, "4.4 Glossary", level=2)
    glossary = [
    ...
    _heading(doc, "4.5 Assumptions and Limitations", level=2)
    doc.add_paragraph(
