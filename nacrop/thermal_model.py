...
dataset (data/nacrop_direct_water_balance.csv, 2000-01-01 to 2027-05-18, 10,000
...
DATA_CSV = os.path.join(DATA_DIR, "nacrop_direct_water_balance.csv")
...
def multi_method_et(current_temp_c: float, current_rh_pct: float, as_of: Optional[date] = None,
    ...
    FAO-56 stage curve for any other crop in nacrop.crops -- see crops.py for how
...
def build_temperature_anchored_season(current_temp_c: float, current_rh_pct: float,
    ...
    nacrop.crops for any other crop), with TODAY's slot replaced by the ensemble
