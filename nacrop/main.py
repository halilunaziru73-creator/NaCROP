...
Run:  python -m nacrop.main --mode compare
      python -m nacrop.main --mode indirect --et-method fao56
      python -m nacrop.main --mode direct
"""
import argparse
import json
import os
...
def run_direct_method(data=None):
    ...
    Direct method per the water-balance equation:
        ETc = I + P + W - RO - DP (+/-) dS
    Now genuinely computable: data/nacrop_direct_water_balance.csv provides all six
    ...
    df = pd.read_csv(os.path.join(os.path.dirname(os.path.dirname(__file__)), "data",
                                   "nacrop_direct_water_balance.csv"), parse_dates=["date"])
