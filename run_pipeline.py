...
run_pipeline.py
----------------
End-to-end driver: runs the full NaCROP pipeline, exports tables (CSV), figures
...
from nacrop import soil_water as sw
from nacrop import irrigation as irr
from nacrop import config as cfg
from nacrop import interpolation as interp
...
def main():
    ...
    # ---------------- NEW: NaCROP Simulation stage ----------------
    # Added AFTER every existing stage, so the original pipeline order is preserved.
    # Non-interactive here (ask_questions=False) so the CLI run never blocks waiting
    # for input; the GUI's own "Simulation" tab asks the same questions interactively.
    from nacrop import simulation as sim
    from nacrop import simulation_figure as simfig
    from nacrop import thermal_model as tm
    crop_key = "maize"  # this CLI driver runs the fixed field-dataset (maize-only) pipeline
    thermal = tm.load_or_build_model()
    sim_result = sim.simulate_nacrop_season(
        crop_key, thermal.doy_climatology, cfg.DEFAULT_SITE.latitude_deg,
        farm_area_ha=1.0, dwb_depletion_series=[d.depletion_mm for d in downstream["dwb"]],
        dwb_taw_series=downstream["taw_raw"]["TAW_mm"], ask_questions=False,
    )
    sim_fig_path = simfig.plot_nacrop_simulation(sim_result)
    with open(os.path.join(TAB, "nacrop_simulation_result.json"), "w") as f:
        json.dump({
            "crop": sim_result.crop_name, "season_length_days": sim_result.season_length_days,
            "params": sim_result.params, "final_biomass_kg_ha": sim_result.final_biomass_kg_ha,
            "reference_hi_pct": sim_result.reference_hi_pct, "actual_hi_pct": sim_result.actual_hi_pct,
            "yield_kg_ha": sim_result.yield_kg_ha, "yield_t_ha": sim_result.yield_t_ha,
        }, f, indent=2)
    md += (
        "\n\n## 11. NaCROP Simulation\n"
        f"See `{os.path.basename(sim_fig_path)}` and `tables/nacrop_simulation_result.json`. "
        f"Default crop-science parameters were used (non-interactive CLI run); the GUI's "
        f"Simulation tab asks for farm-specific values instead.\n\n"
        f"- Final biomass: **{sim_result.final_biomass_kg_ha:,.0f} kg/ha**\n"
        f"- Reference HIo: **{sim_result.reference_hi_pct}%**  |  Actual (stress-adjusted) HI: "
        f"**{sim_result.actual_hi_pct}%**\n"
        f"- Simulated yield: **{sim_result.yield_kg_ha:,.0f} kg/ha** "
        f"({sim_result.yield_t_ha:.2f} t/ha)\n"
    )
    ...
    print(f"\nNaCROP Simulation: {sim_result.crop_name} -> "
          f"{sim_result.yield_kg_ha:,.0f} kg/ha ({sim_result.yield_t_ha:.2f} t/ha)")
